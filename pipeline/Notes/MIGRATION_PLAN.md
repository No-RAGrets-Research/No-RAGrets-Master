...
│   └── synthesizer.txt
└── utils/
    ├── text_loader.py         # Docling PDF to markdown
    ├── llm_runner.py          # OpenAI/Ollama client
    ├── result_merger.py       # Concatenate rubric outputs
...
Input (PDF/Text) → 6 Rubric Evaluations → Merge → Meta-Synthesis → Final Review
...
│   └── synthesizer.txt
└── utils/
    ├── text_loader.py         # PDF → markdown conversion
    ├── llm_runner.py          # LLM client abstraction
    ├── result_merger.py       # Output consolidation

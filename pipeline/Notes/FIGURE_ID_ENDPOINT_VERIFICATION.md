...
### 1. Visual Extraction PipelineOK
...
OK Schema (`schema.graphql`) - has `figure_id: String @search(by: [term])`
OK API endpoint (`api.py`) - `/api/relations/by-figure` implemented
OK Data loader (`kg_data_loader.py`) - extracts and stores `figure_id`
OK Visual extraction pipeline - creates `figure_id` in source_span
OK Database storage - `figure_id` persisted and queryable

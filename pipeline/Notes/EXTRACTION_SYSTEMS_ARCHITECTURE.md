...
[Stage 1] Docling PDF Parsing
    ...
    ├─ Figure detection and metadata extraction
    ├─ Table detection and structure parsing
    └─ Output: Structured JSON with all content types
    ↓
[Stage 2] Text Chunking
    ├─ Segment text into processable chunks
    ├─ Preserve section structure
    ├─ Sentence segmentation using spaCy
    └─ Output: Text chunks with sentence-level offsets
    ↓
┌─────────────────────────────────────────────────────┐
│        PARALLEL EXTRACTION SYSTEMS                  │
├─────────────────────────────────────────────────────┤
│                                                     │
│  [Text Extraction]     [Visual Extraction]         │
│         ↓                      ↓                    │
│   Text Triples           Visual Triples            │
│                                                     │
│            [Table Extraction]                       │
│                    ↓                                │
│              Table Triples                          │
...
```
Headers: Site | CH4 Flux (mg m-2 d-1) | Temperature (°C) | Moisture (%)
...
┌─────────────────────────────────────────────────────────────┐
│               Docling PDF Parsing                           │
│  ├─ Text elements (paragraphs, sections)                    │
│  ├─ Figure metadata (captions, locations)                   │
│  └─ Table structures (cells, headers)                       │
└────────┬──────────────┬──────────────┬─────────────────────┘
         ↓              ↓              ↓
    ┌────────┐    ┌─────────┐   ┌──────────┐
    │  Text  │    │ Figures │   │  Tables  │
    ...
    └───┬────┘    └────┬────┘   └────┬─────┘
        ↓              ↓              ↓
┌───────────────┐ ┌─────────────┐ ┌──────────────┐
│Text Chunking  │ │Image Extract│ │Linearization │
│+ spaCy Segmen │ │+ Matching   │ │+ spaCy Segmen│
└───────┬───────┘ └──────┬──────┘ └──────┬───────┘
        ↓                ↓                ↓
┌───────────────┐ ┌─────────────┐ ┌──────────────┐
│   Mistral 7B  │ │  Qwen3-VL   │ │  Mistral 7B  │
│  LLM Extract  │ │VLM Extract  │ │  LLM Extract │
└───────┬───────┘ └──────┬──────┘ └──────┬───────┘
        ↓                ↓                ↓
┌───────────────┐ ┌─────────────┐ ┌──────────────┐
│  Span Mapping │ │Visual Spans │ │Table Spans   │
└───────┬───────┘ └──────┬──────┘ └──────┬───────┘
        ↓                ↓                ↓
        └────────────────┴────────────────┘
                         ↓
...
│  ├─ Relation insertion with provenance                      │
...
│  ├─ 14,900 relations (current corpus)                       │
│  ├─ Multiple source types per paper                         │
│  ├─ Full provenance chains                                  │
│  └─ API-accessible with docling_ref                         │
└─────────────────────────────────────────────────────────────┘
```

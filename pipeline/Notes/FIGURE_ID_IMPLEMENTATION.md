...
✅ Added `figure_id` field to Relation type:
...
✅ Added new endpoint `/api/relations/by-figure`
✅ Updated `RelationResponse` model to include `figure_id`

### 3. Data Loader (`knowledge_graph/kg_data_loader.py`)

✅ Modified `create_relation()` to accept `figure_id` parameter
✅ Updated GraphQL mutation to include `figure_id` field
✅ Extracts `figure_id` from source_span when `span_type == 'visual_figure'`
✅ Passes `figure_id` to database during relation creation

### 4. Documentation (`knowledge_graph/API_ENDPOINTS.md`)

✅ Added full documentation for `/api/relations/by-figure` endpoint
...
✅ **Easy filtering**: Know which relations came from figures vs text
✅ **Quality tracking**: Monitor visual extraction vs text extraction separately
✅ **Selective re-processing**: Re-run just visual or just text extraction
✅ **Clear provenance**: Users can see "this came from Figure 3"
✅ **Better debugging**: Isolate issues in visual vs text pipeline

## File Structure

```
output/
├── text_kg_results/     ← Text extraction (no figure_id)
│   └── paper_text.json
└── visual_kg_results/   ← Visual extraction (has figure_id)
    └── paper_visual.json
```

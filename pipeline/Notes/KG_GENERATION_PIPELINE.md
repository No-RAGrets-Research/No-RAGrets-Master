...
[2] Text Chunking with Provenance
    ↓
[3] Text-based KG Extraction ──→ Text Triples Output
    ↓
[4] Figure Detection
    ↓
[5] Visual KG Extraction ──→ Visual Triples Output
    ↓
...
[Stage 1] Docling Conversion
    ...
    ├─ Figure identification and metadata
    └─ Output: Docling JSON with full document structure
    ↓
[Stage 2] Text Chunking + Sentence Segmentation
    ├─ Load spaCy en_core_web_sm model
    ├─ Section identification (Introduction, Methods, etc.)
    ├─ Paragraph splitting
    ├─ Sentence segmentation using spaCy NLP:
    │   • doc = nlp(text)
    │   • for sent in doc.sents: ...
    │   • Extract sent.start_char, sent.end_char
    ├─ Calculate document-level character offsets
    ├─ Track page numbers and bounding boxes
    ├─ Build sentence provenance metadata
    └─ Output: Text chunks JSONL with sentence-level provenance
    ↓
[Stage 3] Text-based KG Extraction
    ├─ LLM processing (Mistral 7B via Ollama)
    ├─ Entity extraction from chunks
    ├─ Relation extraction with context
    ├─ Map relations back to specific sentences (using spaCy offsets)
    ├─ Calculate confidence scores
    └─ Output: Text triples with full provenance
    ↓
[Stage 4] Figure Detection
    ├─ Analyze Docling JSON for extractable figures
    ├─ Count figures with valid page numbers
    ├─ Filter non-extractable images
    └─ Decision: Proceed to visual extraction if figures present
    ↓
[Stage 5] Visual KG Extraction (if figures detected)
    ├─ Extract images from PDF using fitz
    ├─ Match images to Docling figure metadata
    ├─ Vision-language model processing (Qwen3-VL)
    ├─ Extract visual entities and relationships
    ├─ Add figure-specific provenance
    └─ Output: Visual triples with figure IDs
    ↓
[Stage 6] Optional: Table-Only Extraction
    ├─ Load table elements from Docling JSON
    ├─ Create chunks from table content
    ├─ Sentence segmentation using spaCy (same as Stage 2)
    ├─ LLM extraction on table chunks
    ├─ Add table-specific provenance (table_id)
    └─ Output: Table triples (supplemental)
```
...
output/
└── table_triples/
    └── research_paper_table_kg_TIMESTAMP.json
```
...
```json
{
  ...
  "table_id": "Table_2",
  "source_span": {
    "text_evidence": "CH4 flux increased from 45 to 120 mg m-2 d-1 with temperature rise from 15°C to 25°C",
    "char_positions": { "start": 23, "end": 109 },
    "sentences": [
      "CH4 flux increased from 45 to 120 mg m-2 d-1 with temperature rise from 15°C to 25°C"
    ]
  },
  "confidence": 0.92
}

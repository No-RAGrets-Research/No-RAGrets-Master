...
for relation in chunk.get('relations', []):
    ...
    if isinstance(relation, list) and len(relation) >= 3:
        ...
        source_span_data = relation.get('source_span')  # ← Only defined here!
...
figure_id = None
if source_span_data and source_span_data.get('span_type') == 'visual_figure':  # ← Error!
...
## Impact
- ✅ Visual triples now correctly store `figure_id` in the database
- ✅ The `/api/relations/by-figure` endpoint can filter relations by figure/table
- ✅ Users can now query "which relations came from this specific figure?"
- ✅ Supports distinguishing between text-extracted and visually-extracted knowledge

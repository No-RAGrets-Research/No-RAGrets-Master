...
- `span_type`: Type of span ("single_sentence" or "multi_sentence")
- `text_evidence`: The complete text containing the relation
- `confidence`: Confidence score (0.0 - 1.0) for the extraction
- `docling_ref`: **[NEW]** Direct reference to the Docling JSON element (e.g., "#/texts/19", "#/pictures/0", "#/tables/0")
  - Enables direct DOM selection in frontend with `querySelector('[data-docling-ref="#/texts/19"]')`
  - Eliminates need for fuzzy text matching
  - Available for all 14,900+ relations in the database
- `location`: Position information within the document
  - `chunk_id`: Text chunk identifier
  - `sentence_range`: [start, end] sentence indices
  - `document_offsets`: Character positions in the full document
- `subject_positions`: Array of subject entity locations in the text
- `object_positions`: Array of object entity locations in the text
...
- **Provenance fields** (Relations track where they came from):
  ...
  - `source_span`: Detailed text span with character positions and Docling reference
  - `docling_ref`: Direct reference to source element (e.g., "#/texts/19") - included in source_span
...
### 3. Frontend Integration: Navigate from Graph to Paper

```bash
# Step 1: User clicks a relation in the knowledge graph UI
# Get the relation's source span with docling_ref
curl "http://localhost:8001/api/relations/0x45ae/source-span"

# Response includes docling_ref:
# {
#   "source_span": {
...
# Step 2: Frontend uses docling_ref for direct DOM selection
const element = document.querySelector('[data-docling-ref="#/texts/209"]');
element.scrollIntoView({ behavior: 'smooth', block: 'center' });

# Benefits:
# - No fuzzy text matching required
# - Instant, reliable scrolling
# - Works for text, figures, and tables
# - All 14,900+ relations have docling_ref
```

---

### 4. Analyze Knowledge Graph Structure

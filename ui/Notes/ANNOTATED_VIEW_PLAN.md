...
```
Docling JSON (local) → Parse & Render HTML → User Clicks → API Lookup → Display Relations
                              ↓                                              ↓
...
4. **Clear selection action:**
   - Click X button → deselect → return to page view
   - Click elsewhere in document → new selection → fetch new relations
   - Switch pages → auto-clear selection
...
- Citation linking (click references → jump to bibliography)
...
│  Abstract                            │
│  ──────────────────────────────────  │
│  Methanotrophs convert methane to    │ ← Click text block
│  methanol using MMO enzymes...       │   → fetch relations
│                                      │   → show in sidebar
│  3.3 Methanol Production             │
│  ──────────────────────────────────  │
│  The conversion process requires...  │
│                                      │
│  [Figure 1: MMO structure]           │ ← Click figure
│   ↑ Clickable figure placeholder     │   → fetch figure relations
└─────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────┐
│  Methanotrophs convert methane to    │ ← Light blue highlight
│  methanol using MMO enzymes...       │   Cursor: pointer
└─────────────────────────────────────┘
```
...
✅ **Instant loading** - No API calls until user clicks (0ms initial load)  
✅ **Complete document structure** - Sections, figures, tables, captions from docling  
✅ **Efficient exploration** - Only fetch relations for content you interact with  
✅ **Clean reading experience** - Proper typography, semantic HTML  
✅ **Click-to-discover** - Explore relations by clicking any text or figure  
✅ **Natural interactions** - Hover, click, scroll just like any webpage

### For Development

✅ **Simple architecture** - No PDF extraction, no text matching, no coordinate conversion  
✅ **Reliable data** - Docling pre-processed, no runtime parsing errors  
✅ **Fast API** - New endpoints optimized for lookup patterns  
✅ **Easy to test** - Standard React components, no canvas complexity  
✅ **Better performance** - Local JSON parsing vs slow API calls  
✅ **Maintainable** - Clear separation: render docling → handle clicks → fetch relations

### For API Efficiency

✅ **Lazy loading** - Only fetch relations when user clicks (vs all upfront)  
✅ **Targeted queries** - New endpoints allow precise lookups (by-chunk, by-figure, by-text)  
✅ **Reduced load** - 1-2 API calls per click vs 1000+ on page load  
✅ **Graceful fallbacks** - Try chunk → text → location in sequence  
✅ **Batch operations** - source-spans endpoint supports 500 IDs when needed

### For Future Features

✅ **Cross-paper comparison** - Side-by-side docling views  
✅ **Custom annotations** - User highlights stored separately  
✅ **Export** - Markdown, HTML with relations embedded  
✅ **Mobile-friendly** - Responsive HTML, no canvas  
✅ **Knowledge graph** - Click entity → visualize connections  
✅ **Search** - Full-text search in docling JSON (instant)
...
- Reference view (PDF viewer) stays unchanged ✅
- Sidebar components reused (RelationDetail, EntityBreakdown) ✅
...
- Implement click handlers (chunk → text → location fallbacks)
...
- Docling JSON missing → graceful error, offer PDF view only
- API endpoint fails → show toast, try next fallback
- No relations found → show "No relations extracted from this section"
- Malformed docling → log error, render what's parseable
...
1. **Finalize this plan** ✅ (current step - update complete)

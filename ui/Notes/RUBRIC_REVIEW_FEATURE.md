...
└── Sidebar
    ├── SelectionHeader (with "Review" button + X button)
    └── Relations List
...
1. User selects sentences → selectedContent exists
2. "Review" button appears in sidebar SelectionHeader
3. User clicks "Review" → dropdown menu shows 6 rubric options
...
7. Response arrives (~30 seconds) → modal shows review text
8. User closes modal OR selects new text → review state clears
9. If error occurs → show error message with retry button in modal
